...
def main_menu():
    ...
    builder.button(text="📅 Калькулятор срока", callback_data="calc_menu")
    builder.button(text="📋 Права осуждённых", callback_data="rights")
    builder.button(text="📝 Шаблоны документов", callback_data="templates")
    builder.button(text="📞 Полезные контакты", callback_data="contacts")
...
def calc_menu():
    builder = InlineKeyboardBuilder()
    builder.button(text="🔢 Расчёт УДО", callback_data="calc_udo")
    builder.button(text="🔢 Замена наказания", callback_data="calc_replace")
    builder.button(text="🔢 Срок обжалования", callback_data="calc_appeal")
...
def back_button():
    builder = InlineKeyboardBuilder()
    builder.button(text="⬅️ Назад в меню", callback_data="main")
    return builder.as_markup()
...
async def show_calc_menu(callback: CallbackQuery):
    await callback.message.edit_text(
        ...
        parse_mode="HTML", reply_markup=calc_menu()
    )
    await callback.answer()

@dp.callback_query(F.data == "calc_udo")
...
@dp.callback_query(F.data == "calc_replace")
...
@dp.callback_query(F.data == "calc_appeal")
...
@dp.callback_query(F.data == "rights")
